...
class TableList extends PureComponent {
  ...
  handleAdd = fields => {
    ...
    dispatch({
      ...
      const { machineManage: { queryResult } } = this.props;
  ...
  render () {
    ...
    const menu = (
      <Menu onClick={this.handleMenuClick} selectedKeys={[]}>
        <Menu.Item key="remove" disabled={!hasAuthority('Machine_Delete')}>删除</Menu.Item>
        <Menu.Item key="active" disabled={!hasAuthority('Machine_Active')}>批量启用</Menu.Item>
        <Menu.Item key="deactive" disabled={!hasAuthority('Machine_Active')}>批量禁用</Menu.Item>
      </Menu>
    );
