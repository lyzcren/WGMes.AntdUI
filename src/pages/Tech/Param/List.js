...
class TableList extends PureComponent {
  ...
  handleExport = (e) => {
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      var params = this.getSearchParam(fieldsValue);
      var fileName = '工艺参数.xls';
      switch (e.key) {
        case 'currentPage':
          params = { ...params, exportPage: true }
          fileName = '工艺参数-第' + params.pagination.current + '页.xls';
          break;
        case 'allPage':
      ...
      exportExcel('/api/param/export', params, fileName)
    });
  };
  ...
  render () {
    ...
    const { selectedRows, modalVisible, updateModalVisible, updateFormValues, valuesModalVisible } = this.state;
    const menu = (
      <Menu onClick={this.handleMenuClick} selectedKeys={[]}>
        <Menu.Item key="remove" disabled={!hasAuthority('Param_Delete')}>删除</Menu.Item>
        <Menu.Item key="active" disabled={!hasAuthority('Param_Active')}>批量启用</Menu.Item>
        <Menu.Item key="deactive" disabled={!hasAuthority('Param_Active')}>批量禁用</Menu.Item>
      </Menu>
    );
